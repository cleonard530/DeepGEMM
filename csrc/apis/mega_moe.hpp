...
struct SymmBufferLayoutInfo {
    ...
    // Flatten into a plain `int[]` so it can cross the STABLE_TORCH_LIBRARY boundary
...
static void fp8_fp4_mega_moe(
    ...
    DG_HOST_ASSERT(torch_compat::nbytes(sym_buffer) >= static_cast<size_t>(num_required_bytes));
