...
using SymmBufferPlan = std::tuple<int64_t, std::function<SymmBufferSlice(const torch::Tensor&)>>;

static SymmBufferPlan build_symm_buffer_plan(
    const int& num_ranks, const int& num_experts,
...
static int64_t get_symm_buffer_size_for_mega_moe(
    ...
    const std::string& mma_type, const std::string& activation,
    const int& num_shared_experts = 0) {
    return std::get<0>(build_symm_buffer_plan(
...
static SymmBufferSlice slice_symm_buffer_for_mega_moe(
    ...
    const std::string& mma_type, const std::string& activation,
    const int& num_shared_experts = 0) {
    return std::get<1>(build_symm_buffer_plan(
        num_ranks, num_experts, num_max_tokens_per_rank, num_topk,
        hidden, intermediate_hidden, mma_type, activation, num_shared_experts))(buffer);
}
...
static void fp8_fp4_mega_moe(
    ...
    const auto [num_required_bytes, slice] = build_symm_buffer_plan(
...
static int64_t get_block_m_for_mega_moe(
    ...
    const int64_t& num_max_tokens_per_rank, const int64_t& num_tokens, const int64_t& num_topk,
    const std::string& mma_type) {
...
static void fp8_fp4_mega_moe(
    ...
    const bool& fast_math) {
    std::optional<std::tuple<torch::Tensor, torch::Tensor>> shared_l1, shared_l2;
    if (shared_l1_weights.has_value()) {
        DG_HOST_ASSERT(shared_l1_weights_sf.has_value());
        DG_HOST_ASSERT(shared_l2_weights.has_value() and shared_l2_weights_sf.has_value());
        shared_l1 = std::make_tuple(shared_l1_weights.value(), shared_l1_weights_sf.value());
        shared_l2 = std::make_tuple(shared_l2_weights.value(), shared_l2_weights_sf.value());
    }
...
TORCH_LIBRARY_FRAGMENT(deep_gemm, m) {
    ...
    m.def(
        ...
        DEEP_GEMM_IMPL(get_block_m_for_mega_moe));
    m.def(
    ...
    m.def(
        "slice_symm_buffer_for_mega_moe(Tensor buffer, int num_ranks, int num_experts, int num_max_tokens_per_rank, int num_topk, int hidden, int intermediate_hidden, str mma_type, str activation, int num_shared_experts=0) -> (Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor)");
    m.def(
