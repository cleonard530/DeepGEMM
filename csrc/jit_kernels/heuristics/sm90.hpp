...
struct SM90ArchSpec {
    ...
    static StorageConfig get_storage_config(const GemmDesc& desc, const Layout& layout) {
        ...
        const auto swizzle_mode_a = get_swizzle_mode(
            desc.major_a == cute::UMMA::Major::K ? layout.block_k : load_block_m, torch_compat::element_size(desc.a_dtype));
        const auto swizzle_mode_b = get_swizzle_mode(
            desc.major_b == cute::UMMA::Major::K ? layout.block_k : load_block_n, torch_compat::element_size(desc.b_dtype));
        // We only enable swizzling for non-FP32 outputs
        const auto swizzle_mode_cd = desc.cd_dtype != torch::headeronly::ScalarType::Float ?
            get_swizzle_mode(store_block_n, torch_compat::element_size(desc.cd_dtype)) : 0;
    ...
    static PipelineConfig get_pipeline_config(const GemmDesc& desc, const Layout& layout, const StorageConfig& storage_config) {
        ...
        const int smem_cd =
            align(layout.block_m * layout.block_n * static_cast<int>(torch_compat::element_size(desc.cd_dtype)), 1024);
        const int smem_barriers = kNumMaxStages * 8 * 2;

        // Calculate A/B per stages
        const int smem_a_per_stage = storage_config.load_block_m * layout.block_k * torch_compat::element_size(desc.a_dtype);
        const int smem_b_per_stage = storage_config.load_block_n * layout.block_k * torch_compat::element_size(desc.b_dtype);
    ...
    static LayoutInfo get_layout_info(const GemmDesc& desc, const Layout& layout) {
        ...
        const int elem_size_ab = torch_compat::element_size(desc.a_dtype);
        const int elem_size_cd = torch_compat::element_size(desc.cd_dtype);
