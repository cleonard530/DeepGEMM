...
def _unpack_optional_pair(weights):
    if weights is None:
        return None, None
...
def fp8_fp4_mega_moe(y, l1_weights, l2_weights, shared_l1_weights, shared_l2_weights,
                     cumulative_local_expert_recv_stats, sym_buffer, sym_buffer_ptrs, rank_idx,
                     num_max_tokens_per_rank, num_experts, num_topk, recipe, activation,
                     activation_clamp, fast_math):
    shared_l1, shared_l1_sf = _unpack_optional_pair(shared_l1_weights)
    shared_l2, shared_l2_sf = _unpack_optional_pair(shared_l2_weights)
    return _torch_ops.fp8_fp4_mega_moe(
        y, l1_weights[0], l1_weights[1], l2_weights[0], l2_weights[1],
        shared_l1, shared_l1_sf, shared_l2, shared_l2_sf,
...
def bf16_mega_moe(y, l1_weights, l2_weights, shared_l1_weights, shared_l2_weights,
                  cumulative_local_expert_recv_stats, sym_buffer, sym_buffer_ptrs, rank_idx,
                  num_max_tokens_per_rank, num_experts, num_topk, activation, activation_clamp,
                  fast_math):
    return _torch_ops.bf16_mega_moe(
        y, l1_weights, l2_weights, shared_l1_weights, shared_l2_weights,
        cumulative_local_expert_recv_stats, sym_buffer, list(sym_buffer_ptrs), rank_idx,
        num_max_tokens_per_rank, num_experts, num_topk, activation, activation_clamp, fast_math,
    )
