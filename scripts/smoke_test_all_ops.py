...
_C_ONLY_OPS = frozenset({
    'init',
    'get_token_alignment_for_mega_moe', 'get_ring_limit_for_mega_moe',
    'get_symm_buffer_size_for_mega_moe', 'slice_symm_buffer_for_mega_moe',
})
...
def _mega_ring_args():
    ...
    ring = int(_C.get_ring_limit_for_mega_moe(tokens, 8, 2, 1)[0])
    return _C, tokens, ring
...
def build_op_calls() -> dict[str, Callable[[], None]]:
    ...
    def _ring_limit():
        from deep_gemm import _C
        _C.get_ring_limit_for_mega_moe(_C.get_token_alignment_for_mega_moe(), 4, 2, 8)

    def _symm_size():
        _C, tokens, ring = _mega_ring_args()
        _C.get_symm_buffer_size_for_mega_moe(
            1, 8, tokens, 2, 256, 512, 'bf16xbf16', 'swiglu', ring)

    def _slice_buf():
        _require_cuda()
        _C, tokens, ring = _mega_ring_args()
        size = _C.get_symm_buffer_size_for_mega_moe(
            1, 8, tokens, 2, 256, 512, 'bf16xbf16', 'swiglu', ring)
        _C.slice_symm_buffer_for_mega_moe(
            torch.empty(size, device='cuda', dtype=torch.uint8),
            1, 8, tokens, 2, 256, 512, 'bf16xbf16', 'swiglu', ring)

    calls['get_token_alignment_for_mega_moe'] = _token_align
    calls['get_ring_limit_for_mega_moe'] = _ring_limit
    calls['get_symm_buffer_size_for_mega_moe'] = _symm_size
    calls['slice_symm_buffer_for_mega_moe'] = _slice_buf
